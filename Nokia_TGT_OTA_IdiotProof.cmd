@echo off
cls
title TECHGUYTOOL - NOKIA (HMD) OTA IDIOT PROOF
color 0A
...
echo(
echo   Nokia / HMD helper codes (model dependent):
echo     *#*#4636#*#*  (generic)
echo(
echo ===============================================================================
echo(
...
:: ---- TOAST START ----
adb shell am broadcast -a com.android.systemui.toast --es message "TechGuyTool: Locking Nokia OTA..." --ei duration 1 >nul 2>&1

:: ---- CLEAR + DISABLE ----
echo [*] Clearing updater state...
adb shell pm clear com.hmdglobal.ota >nul 2>&1
adb shell pm clear com.android.updater >nul 2>&1

echo [*] Disabling updater services...
adb shell pm disable-user --user 0 com.hmdglobal.ota >nul 2>&1
adb shell pm disable-user --user 0 com.android.updater >nul 2>&1

:: ---- DONE TOAST ----
adb shell am broadcast -a com.android.systemui.toast --es message "TechGuyTool: Nokia OTA - IDIOT PROOF ✅" --ei duration 1 >nul 2>&1
